import ytProxy from "../../../bot/music/youtube-proxy.js";
...
export default class Play extends CustomCommand {
  constructor() {
    super(
      ["play", "p"],
      "COW bot plays the song in the voice channel you're in."
    );
  }
    ...
    if (isYTUrl(split[1])) {
      ...
      ytProxy.search(msg.content.substr(split[0].length + 1), (err, res) => {
        if (err) DiscordUtils.send(err, msg.channel);
          ...
          DiscordUtils.send(
            "Unable to find a video matching that name :(",
            msg.channel
          );
        else {
...
function handleURL(vc, msg, res) {
  global.musicBotManager.addSong(
    vc,
    msg.channel,
    new Song(
