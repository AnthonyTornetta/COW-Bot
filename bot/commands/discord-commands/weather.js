...
export default class Weather extends CustomCommand {
  constructor() {
  ...
  action(msg) {
    ...
    fetch(
      `https://api.darksky.net/forecast/${API_KEY}/${lat},${lon}?units=si`
    ).then((resp) =>
      resp.json().then((data) => {
        let weatherMsg = `== **Currently** ==\nTemperature: ${
          data.currently.temperature
        }°C (${~~Math.round(tF(data.currently.temperature))}°F)\n${
          data.hourly.summary
        ...
        for (let i = 0; i <= 24; i++) {
          ...
          if (time.getHours() > 12) timeStr = (time.getHours() % 12) + "";
          else if (time.getHours() === 0) timeStr = "12";

          weatherMsg += `\n\n**== ${timeStr} ${
            time.getHours() >= 12 ? "PM" : "AM"
          }** ==\nTemperature: ${~~Math.round(temperature)}°C (${~~Math.round(
            tF(temperature)
          )}°F)\nSummary: ${sum}\nPrecipitation: ${
            precip ? `${precip} ${~~Math.round(prob * 100)}%` : "None"
          }\nWind: ${wind} m/s`;
        }
